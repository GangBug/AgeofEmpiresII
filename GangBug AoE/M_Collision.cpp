		...
		for (int j = 0; j < unit2.size(); j++)
		{
			if (unit1[i] != unit2[j])
			{
				if (DoUnitsIntersect((Unit*)unit1[i], (Unit*)unit2[j]) == true)
				{
					...
					if ((unit_1->GetAction() == IDLE) && (unit_2->GetAction() == IDLE || unit_2->GetAction() == ATTACK) && unit_1->GetUnitType() == SAMURAI)
						SplitUnits((Unit*)unit1[i]);
					if ((unit_1->GetAction() == IDLE) && (unit_2->GetAction() == IDLE || unit_2->GetAction() == ATTACK) && unit_1->GetUnitType() == TARKAN_KNIGHT)
						SplitUnits((Unit*)unit1[i]);
				}
			}
